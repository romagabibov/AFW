...
function fixFile(filePath: string) {
  ...
  do {
    ...
    content = content.replace(/value=\{\$1 \|\| ""\}\s*onChange=\{\(e\) => updateMediaLink\(([a-zA-Z0-9_\.]+),\s*([a-zA-Z0-9_\.]+),\s*"([a-zA-Z0-9_]+)",/g, (match, p1, p2, p3) => {
      let listVar = p2.split('.')[0]; // day ? NO, listVar should be link
      return `value={link.${p3} || ""} onChange={(e) => updateMediaLink(${p1}, ${p2}, "${p3}",`;
    });
  ...
  content = content.replace(/value=\{\$1 \|\| ""\}\s*onChange=\{\(e\) => updateMediaDay\(([a-zA-Z0-9_\.]+),\s*"([a-zA-Z0-9_]+)",/g, (match, p1, p2) => {
      let listVar = p1.split('.')[0]; // day
      return `value={${listVar}.${p2} || ""} onChange={(e) => updateMediaDay(${p1}, "${p2}",`;
  });

  content = content.replace(/value=\{\$1 \|\| ""\}\s*onChange=\{\(e\) => updateSchedule\(([a-zA-Z0-9_\.]+),\s*"([a-zA-Z0-9_]+)",/g, (match, p1, p2) => {
      let listVar = p1.split('.')[0]; // item
      return `value={${listVar}.${p2} || ""} onChange={(e) => updateSchedule(${p1}, "${p2}",`;
  });
  
  content = content.replace(/value=\{\$1 \|\| ""\}\s*onChange=\{\(e\) => updateNews\(([a-zA-Z0-9_\.]+),\s*"([a-zA-Z0-9_]+)",/g, (match, p1, p2) => {
      let listVar = p1.split('.')[0]; // newsItem
      return `value={${listVar}.${p2} || ""} onChange={(e) => updateNews(${p1}, "${p2}",`;
  });
